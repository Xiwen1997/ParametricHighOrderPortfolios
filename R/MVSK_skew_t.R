...
#' xi <- 10
#' lambda <- c(1, 4, 10, 20)
#' MVSK_portfolio <- solve_MVSK_skew_t(lambda, parameters, method = "RFPA", tau = 10)
#'
#' @importFrom utils tail
#' @import quadprog
#' @export
solve_MVSK_skew_t <- function(lambda, parameters,
                              w_init = rep(1/length(parameters$mu), length(parameters$mu)),
  ...
  fun_eval <- function(w) {
    ...
    if (method == "Q-MVSK") {
      return_list$H2 <- 2 * (parameters$a$a21 * parameters$scatter + parameters$a$a22 * parameters$gamma %*% t(parameters$gamma))
      return_list$H3 <- (parameters$a$a31 * 6 * as.numeric(t(w) %*% parameters$gamma) * parameters$gamma %*% t(parameters$gamma) + parameters$a$a32 * (2 * parameters$gamma %*% t(w) %*% parameters$scatter + 2 * parameters$scatter %*% w %*% t(parameters$gamma) + 2 * as.numeric(t(w) %*% parameters$gamma) * parameters$scatter ))
      return_list$H4 <- (12 * parameters$a$a41 * (as.numeric(t(w) %*% parameters$gamma)**2) * parameters$gamma %*% t(parameters$gamma)  + 2 * parameters$a$a42 * (2 * as.numeric(t(w) %*% parameters$gamma) * parameters$scatter %*% w %*% t(parameters$gamma) + ((as.numeric(t(w) %*% parameters$gamma))**2) * parameters$scatter + 2 * as.numeric(t(w) %*% parameters$gamma) * parameters$gamma %*% t(w) %*% parameters$scatter + as.numeric(t(w) %*% parameters$scatter %*% w) * parameters$gamma %*% t(parameters$gamma)    ) + 4 * parameters$a$a43 * (as.numeric(t(w) %*% parameters$scatter %*% w) * parameters$scatter + 2 * parameters$scatter %*% w %*% t(w) %*% parameters$scatter))
      return_list$jac <- rbind(as.vector(parameters$mu + parameters$a$a11 * parameters$gamma),
      ...
      return_list$jac <- rbind("grad1" = as.vector(parameters$mu + parameters$a$a11 * parameters$gamma),
                               "grad2" = as.vector(2 * parameters$a$a21 * parameters$scatter %*% w + parameters$a$a22 * 2 * as.numeric(wT_gamma) * as.matrix(parameters$gamma)),
                               "grad3" = as.vector(parameters$a$a31 * 3 * (as.numeric(wT_gamma) ** 2) * as.matrix(parameters$gamma) + parameters$a$a32 * (as.numeric(t(w) %*% (parameters$scatter) %*% w) * as.matrix(parameters$gamma) + 2 * as.numeric(wT_gamma) * parameters$scatter %*% w )),
                               "grad4" = as.vector(4 * parameters$a$a41 * as.numeric((wT_gamma) ** 3) *as.matrix(parameters$gamma) + parameters$a$a42 * (2 * as.numeric((wT_gamma) ** 2) * parameters$scatter %*% w + 2 * as.numeric(wT_Sigma_w) * as.numeric((wT_gamma)) *as.matrix(parameters$gamma)) + 4 * parameters$a$a43 * (as.numeric(wT_Sigma_w)) * (parameters$scatter) %*% w))
      return_list$obj <- sum(lambda * as.vector(return_list$jac %*% w) / c(-1, 2, -3, 4))
    }
  ...
  if (method == "L-MVSK" || method == "DC") {
    ...
    Compute_bounds <- function(parameters) {
      b2 <- 2 * norm(parameters$a$a21 * parameters$scatter + parameters$a$a22 * parameters$gamma %*% t(parameters$gamma) ,"I")

      H3 <- function(w, parameters) {
        H3_mat <-  parameters$a$a31 * 6 * as.numeric(t(w) %*% parameters$gamma) * parameters$gamma %*% t(parameters$gamma) + parameters$a$a32 * (2 * parameters$gamma %*% t(w) %*% parameters$scatter + 2 * parameters$scatter %*% w %*% t(parameters$gamma) + 2 * as.numeric(t(w) %*% parameters$gamma) * parameters$scatter )
        return(H3_mat)
      }
      H4 <- function(w, parameters) {
        H4_mat <- 12 * parameters$a$a41 * (as.numeric(t(w) %*% parameters$gamma)**2) * parameters$gamma %*% t(parameters$gamma)  + 2 * parameters$a$a42 * (2 * as.numeric(t(w) %*% parameters$gamma) * parameters$scatter %*% w %*% t(parameters$gamma) + ((as.numeric(t(w) %*% parameters$gamma))**2) * parameters$scatter + 2 * as.numeric(t(w) %*% parameters$gamma) * parameters$gamma %*% t(w) %*% parameters$scatter + as.numeric(t(w) %*% parameters$scatter %*% w) * parameters$gamma %*% t(parameters$gamma)    ) + 4 * parameters$a$a43 * (as.numeric(t(w) %*% parameters$scatter %*% w) * parameters$scatter + 2 * parameters$scatter %*% w %*% t(w) %*% parameters$scatter)
        return(H4_mat)
      }
      H4_variant <- function(w1, w2, parameters) {
        H4_variant_mat <- 12 * parameters$a$a41 * (as.numeric(t(w1) %*% parameters$gamma) * as.numeric(t(w2) %*% parameters$gamma)) * parameters$gamma %*% t(parameters$gamma)  + 2 * parameters$a$a42 * ( as.numeric(t(w1) %*% parameters$gamma) * parameters$scatter %*% w2 %*% t(parameters$gamma) + as.numeric(t(w1) %*% parameters$gamma) * parameters$scatter %*% w1 %*% t(parameters$gamma) + ((as.numeric(t(w1) %*% parameters$gamma)) * (as.numeric(t(w2) %*% parameters$gamma)) ) * parameters$scatter + as.numeric(t(w1) %*% parameters$gamma) * parameters$gamma %*% t(w2) %*% parameters$scatter + as.numeric(t(w2) %*% parameters$gamma) * parameters$gamma %*% t(w1) %*% parameters$scatter + as.numeric(t(w1) %*% parameters$scatter %*% w2) * parameters$gamma %*% t(parameters$gamma)    ) + 4 * parameters$a$a43 * (as.numeric(t(w1) %*% parameters$scatter %*% w2) * parameters$scatter + parameters$scatter %*% w1 %*% t(w2) %*% parameters$scatter+ parameters$scatter %*% w2 %*% t(w1) %*% parameters$scatter)
      ...
      for(i in 1:N) {
        w_i <- w_b
        w_i[i] <- 1
        H3_mat <- pmax(H3_mat,  abs(H3(w_i, parameters)))
      }
      b3 <- norm(H3_mat, "I")
      ...
      for(i in 1:N) {
        for(j in 1:N) {
          ...
          w_j <- w_b
          w_j[j] <- 1
          H4_mat_variant <- pmax(H4_mat_variant, abs(H4_variant(w_i, w_j, parameters)))
        }
      }
  ...
  if (method == "L-MVSK") {
    bounds <- Compute_bounds(parameters)
    rho <- lambda[3] * bounds$b3 + lambda[4] * bounds$b4
    H2 <- 2 * (parameters$a$a21 * parameters$scatter + parameters$a$a22 * parameters$gamma %*% t(parameters$gamma))
  }
  if (method == "DC") {
    bounds <- Compute_bounds(parameters)
  ...
  for (iter in 1:maxiter) {
    ...
    switch(method,
           ...
           "L-MVSK" = {
             ...
             qk <- rho * wk - (- lambda[1] * (parameters$mu + parameters$a$a11 * parameters$gamma) - lambda[3] * (parameters$a$a31 * 3 * (as.numeric(t(wk) %*% parameters$gamma) ** 2) * as.matrix(parameters$gamma) + parameters$a$a32 * (as.numeric(t(wk) %*% (parameters$scatter) %*% wk) * as.matrix(parameters$gamma) + 2 * as.numeric(t(wk) %*% parameters$gamma) * parameters$scatter %*% wk )) + lambda[4] * (4 * parameters$a$a41 * as.numeric((t(wk) %*% parameters$gamma) ** 3) *as.matrix(parameters$gamma) + parameters$a$a42 * (2 * as.numeric((t(wk) %*% parameters$gamma) ** 2) * parameters$scatter %*% wk + 2 * as.numeric(t(wk) %*% (parameters$scatter) %*% wk) * as.numeric((t(wk) %*% parameters$gamma)) *as.matrix(parameters$gamma)) + 4 * parameters$a$a43 * (as.numeric(t(wk) %*% (parameters$scatter) %*% wk)) * (parameters$scatter) %*% wk   ))
